...
public class EGLPosterRenderer {
    private static final String TAG = EGLPosterRenderer.class.getName();
    ...
    public void into(Surface surface) {
        new EGLPosterRendererThread(mBitmap, recycleBitmap, surface).start();
    }

    public void into(SurfaceTexture surfaceTexture) {
        new EGLPosterRendererThread(mBitmap, recycleBitmap, surfaceTexture).start();
    }
}
